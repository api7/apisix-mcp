...
export const RouteSchema = z
  .object({
    ...
    labels: z.record(z.string(), z.string()).optional().describe("route labels"),
    ...
    remote_addrs: z.array(z.string()).optional().describe("allowed remote addresses"),
    methods: z
      .array(z.enum(["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS", "TRACE", "CONNECT", "PURGE"]))
      .optional()
      .describe("allowed HTTP methods"),
    priority: z.number().optional().describe("route priority").default(0),
    vars: z.array(z.array(z.any())).optional().describe("route match variables"),
    filter_func: z.string().optional().describe("route filter function"),
    script: z.record(z.string(), z.any()).optional().describe("route script configuration"),
    service_id: z.string().optional().describe("service id"),
    upstream_id: z.string().optional().describe("upstream id"),
    upstream: UpstreamSchema.partial().optional().describe("upstream configuration"),
