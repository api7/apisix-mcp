...
const ipv6Pattern = /^([a-fA-F0-9]{0,4}:){1,8}(:[a-fA-F0-9]{0,4}){0,8}([a-fA-F0-9]{0,4})?\/[0-9]{1,3}$/;

const IPAddressSchema = z.union([
  z.string().describe("IPv4"),
  z.string().regex(ipv4Pattern).describe("IPv4/CIDR"),
  z.string().describe("IPv6"),
  z.string().regex(ipv6Pattern).describe("IPv6/CIDR"),
]);

const ProtocolLoggerSchema = z.object({
  name: z.string(),
  conf: z.record(z.string(), z.any()).describe("logger plugin configuration").optional(),
  filter: z.array(z.string()).describe("logger filter rules").optional(),
});

const ProtocolSchema = z.object({
  name: z.string(),
  conf: z.record(z.string(), z.any()).describe("protocol-specific configuration").optional(),
